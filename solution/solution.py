...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for k, chunk in enumerate(contexts, start=1):
            ...
            if not chunk_tokens:
                ...
                relevance_ratio = overlap / len(expected_tokens)
                relevant_k = relevance_ratio >= relevance_threshold
    ...
    def run_full_eval(
        ...
        # Create a dummy QAPair for EvalResult
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"AI Answer: {answer}\n\n"
            f"Evaluation Rubric:\n{rubric_lines}\n\n"
            f"Return your evaluation as JSON with the following structure:\n"
            f'{{"scores": {{"accuracy": 0.85, "clarity": 0.90}}, "reasoning": "explanation here"}}\n'
        ...
        # Fallback: default 0.5 for missing criteria
        if not scores:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for i, batch_item in enumerate(scores_batch):
            if "scores" in batch_item and isinstance(batch_item["scores"], dict):
                for criterion, score in batch_item["scores"].items():
                    all_scores.append(float(score))
                    if i == 0:
                        first_position_scores.append(float(score))
                    else:
                        other_position_scores.append(float(score))

        # Positional bias: first position avg vs others
        positional_bias = False
        if first_position_scores and other_position_scores:
            first_avg = sum(first_position_scores) / len(first_position_scores)
            others_avg = sum(other_position_scores) / len(other_position_scores)
            if first_avg > others_avg + 0.1:  # first is notably higher
                positional_bias = True

        # Leniency bias: avg > 0.8
        leniency_bias = False
        if all_scores:
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        def avg_f(results):
            return sum(r.faithfulness for r in results) / len(results) if results else 0.0

        def avg_r(results):
            return sum(r.relevance for r in results) / len(results) if results else 0.0

        def avg_c(results):
            return sum(r.completeness for r in results) / len(results) if results else 0.0

        new_avg_f = avg_f(new_results)
        new_avg_r = avg_r(new_results)
        new_avg_c = avg_c(new_results)
        baseline_avg_f = avg_f(baseline_results)
        baseline_avg_r = avg_r(baseline_results)
        baseline_avg_c = avg_c(baseline_results)
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            fix = suggestions[i] if i < len(suggestions) else (suggestions[-1] if suggestions else "Review pipeline")
            lines.append(f"| {fid} | {ft} | {rc} | {fix} | Open |")
    ...
    def generate_improvement_suggestions(
        ...
        # Based on failure type distribution
        if categories.get("hallucination", 0) > 0:
            suggestions.append(
                "Implement hallucination guardrail: add a grounding check that flags claims not present in retrieved context before returning the answer"
            )
        if categories.get("irrelevant", 0) > 0:
            suggestions.append(
                "Improve prompt clarity: add explicit instruction to address the exact question intent, with few-shot examples of on-topic vs off-topic responses"
            )
        if categories.get("incomplete", 0) > 0:
            suggestions.append(
                "Add few-shot examples demonstrating complete answers that include all conditions, exceptions, and steps from the expected answer"
            )
        if categories.get("off_topic", 0) > 0:
            suggestions.append(
                "Strengthen intent detection: add a routing layer that detects off-topic queries and either refuses politely or redirects to relevant scope"
            )

        # Always include general suggestions
        if len(suggestions) < 3:
            suggestions.extend([
                "Increase chunk size or overlap in the RAG pipeline to reduce context fragmentation that causes incomplete answers",
                "Implement a reranker (e.g. cross-encoder) to improve Context Precision by placing relevant chunks before noise",
            ])
